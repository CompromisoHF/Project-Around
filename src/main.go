...
>       POST_TYPE   = "post"
>       DISTANCE    = "200km"
>       ES_URL      = "http://35.231.17.254:9200"
>       BUCKET_NAME = "post-images-1017"
>       API_PREFIX  = "/api/v1"
)
var (
...
>       //>     r.Handle("/search", jwtMiddleware.Handler(http.HandlerFunc(handlerSearch))).Methods("GET", "OPTIO
NS")
...
>       //>     r.Handle("/cluster", jwtMiddleware.Handler(http.HandlerFunc(handlerCluster))).Methods("GET", "OPT
IONS")
>       //>     http.Handle("/", r)
>       r.Handle(API_PREFIX+"/post", jwtMiddleware.Handler(http.HandlerFunc(handlerPost))).Methods("POST", "OPTIO
NS")
>       r.Handle(API_PREFIX+"/search", jwtMiddleware.Handler(http.HandlerFunc(handlerSearch))).Methods("GET", "OP
TIONS")
...
>       >       panic(err)
>       }
>       //provide information to elasticsearch
>       if !exists {
>       >       mapping := `{
            "mappings": {
                "post": {
...
>       bt_client, err := bigtable.NewClient(ctx, "divine-glazing-231703", "around-post")
>       if err != nil {
>       >       //panic(err)
>       >       return err
>       }
...
>       >       Index(POST_INDEX).
>       >       Query(query).
>       >       Size(100). // Add this line to increase return item count
>       >       Pretty(true).
>       >       Do(context.Background())
>       if err != nil {
>       >       return nil, err
...
>       }
>       // Range query.
>       // For details, https://www.elastic.co/guide/en/elasticsearch/reference/current/query-dsl-range-query.htm
l
